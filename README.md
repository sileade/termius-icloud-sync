...
Если вы предпочитаете полный контроль над процессом, смотрите подробный гайд: **[Ручная установка (MANUAL_SETUP_GUIDE.md)](MANUAL_SETUP_GUIDE.md)**
...
Проект покрыт набором unit-тестов для проверки ключевой логики фильтрации и извлечения хостов. Тесты запускаются для проверки различных сценариев, включая:

-   Корректное игнорирование хостов с маркером `# termius:ignore`.
-   Сохранение глобальных настроек SSH.
-   Обработка пустых и крайних случаев в конфигурационных файлах.

Все скрипты также проверены статическим анализатором `shellcheck` для обеспечения высокого качества и надежности кода.

Подробный отчет о последнем код-ревью и результатах тестирования можно найти в файле [CODE_REVIEW.md](CODE_REVIEW.md).
